<?php
	...
	if($thisMD5==$prevMD5){
		...
		//**************************** PIRL DASHBOARD TABLES BY OLIVIER MAINA DAMA ******************************///
		setupTable('pirl_dashboard', "create table if not exists `pirl_dashboard` (   `id` INT unsigned not null auto_increment , primary key (`id`), `dashboard` VARCHAR(40) not null default 'Meeting', `category` VARCHAR(40) not null default 'Accelerators Partnership' , `external_entity_name` INT unsigned , `engagement_nature` VARCHAR(40) not null default 'Investigation' , `engagement` VARCHAR(40) not null default 'NA' , `external_contact_name` INT unsigned , `thread` VARCHAR(50) ,`thread_description` TEXT , `thread_status` VARCHAR(40) not null default 'Not started' ,`start_date` DATE , `end_date` DATE , `country` VARCHAR(40), `internal_contact_name` INT unsigned , `usefull_info` TEXT ) CHARSET utf8", $silent);
		setupIndexes('pirl_dashboard', array('internal_contact_name','external_contact_name', 'external_entity_name'));
		setupTable('external_contact', "create table if not exists `external_contact` (   `id` INT unsigned not null auto_increment , primary key (`id`), `last_name` VARCHAR(15) , `first_name` VARCHAR(15) , `poste` VARCHAR(15) , `company_name` VARCHAR(40) ,`email` VARCHAR(80) , `phone` VARCHAR(15) , `alternate_phone` VARCHAR(15), `country` VARCHAR(40) , `info` TEXT ) CHARSET utf8", $silent);
		setupTable('internal_contact', "create table if not exists `internal_contact` (   `id` INT unsigned not null auto_increment , primary key (`id`), `cec` VARCHAR(15),`last_name` VARCHAR(15) , `first_name` VARCHAR(15) , `poste` VARCHAR(15),`email` VARCHAR(80) , `phone` VARCHAR(15) , `country` VARCHAR(40) , `info` TEXT ) CHARSET utf8", $silent);
		setupTable('external_entity', "create table if not exists `external_entity` (   `id` INT unsigned not null auto_increment , primary key (`id`), `company_name` VARCHAR(40)  ,  `company_class` VARCHAR(40)  not null default 'D' , `primary_email` VARCHAR(40) , `alternate_email` VARCHAR(40) , `phone` VARCHAR(40) , `country` VARCHAR(40) , `street` VARCHAR(40) , `city` VARCHAR(40) , `zip` DECIMAL(15,0) , `comments` TEXT ) CHARSET utf8", $silent);
		//************************************* END ADDING **********************//
